...
run_pipeline(mode=args.mode, real=args.real, mission_mode=args.mission_mode, config_overrides=overrides)        # mk: mission=False
    """
    ...
    if mode in ("full", "ingest"):
        ...
        init_db()
        if mission_mode:
            # Mission mode = Rakuten Chalnge with all train data from .csv
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mission_mode", action="store_true",
                            help="mission mode: train on all data")
    ...
    # python -m src.pipeline --mode ingest --mission