...
DRIFT_REPORT = BASE_DIR / "reports" / "data_drift_report_ice_branch.html"
    ...
    # -- API METRICS --
        ...
        with text_col:
            st.markdown(
                ...
                Incremented every time a color is predicted. Tracks production model behavior per color — reveals bias (e.g. always predicting Black, never Yellow). A sudden shift in color distribution signals concept drift.
                </div>
                <div style="background:white; border:1px solid #E2E8F0; border-radius:14px; padding:0.75rem 1rem; margin-bottom:0.5rem;">
                ...
                Tracks every HTTP request. The status_code label is key: 5xx = server errors, 4xx = client errors. The endpoint label identifies which route is failing — /predict vs /predict/upload behave differently.
                </div>
                ...
                Records request duration in seconds. Histograms enable percentile calculation: p50 = typical latency, p95/p99 = tail latency. Averages hide slow outliers — percentiles expose them.
                </div>
                </div>
                """,
                unsafe_allow_html=True
    ...
    # -- TRAINING METRICS --
        ...
        with text_col:
            st.markdown(
                ...
                F1 score per training run. Enables visual side-by-side comparison of all runs — immediately shows whether more epochs or more data improved the model.
                </div>
                <div style="background:white; border:1px solid #E2E8F0; border-radius:14px; padding:0.75rem 1rem; margin-bottom:0.5rem;">
                <b style="color:#0F172A;">rakuten_training_duration_seconds</b> — <span style="font-size:0.90rem;">Labeled by model_version</span><br>
                Training time per run. Tracks efficiency alongside quality — a model with slightly lower F1 but 4x faster training may be the better production choice.
                </div>
                <div style="background:white; border:1px solid #E2E8F0; border-radius:14px; padding:0.75rem 1rem; margin-bottom:0.5rem;">
                <b style="color:#0F172A;">rakuten_champion_f1</b> — <span style="font-size:0.90rem;">Single value</span><br>
                F1 score of the currently promoted champion model. Tracked over time to confirm the pipeline is improving model quality with each run.
                </div>
                <div style="background:white; border:1px solid #E2E8F0; border-radius:14px; padding:0.75rem 1rem;">
                <b style="color:#0F172A;">rakuten_champion_version</b> — <span style="font-size:0.90rem;">Single value</span><br>
                Version number of the champion. Links Grafana back to the exact MLflow run — when F1 changes, this tells you exactly which version caused it.
                </div>
                </div>
                """,
                unsafe_allow_html=True
